...
// --- Fetch filters ---
$categories = mysqli_query($con, "SELECT DISTINCT category FROM stories");
$storytellers = mysqli_query($con, "SELECT DISTINCT storyteller FROM stories");
...
if ($category) $query .= " AND category='" . mysqli_real_escape_string($con, $category) . "'";
if ($storyteller) $query .= " AND storyteller='" . mysqli_real_escape_string($con, $storyteller) . "'";
...
<body class="bg-gradient-to-br from-blue-50 to-yellow-50 min-h-screen">
  <div class="container mx-auto px-4 py-10">
    ...
    <div class="text-center mb-6">
      <a href="myStories.php"
        class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-semibold shadow transition inline-flex items-center gap-2">
    ...
    <form method="get" class="flex flex-wrap gap-4 justify-center mb-8">
      ...
      <select name="storyteller" class="rounded-lg border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-blue-400">
        ...
        <option value="<?= htmlspecialchars($row['storyteller']) ?>"
          <?= $storyteller == $row['storyteller'] ? 'selected' : '' ?>>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
      ...
      <div
        ...
        <img src="<?= htmlspecialchars($row['thumbnail']) ?>" alt="Story"
        ...
        <div class="p-5">
          ...
          <div class="flex items-center justify-between mb-2">
            <span
              class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded"><?= htmlspecialchars($row['category']) ?></span>
            <span
              class="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded"><?= htmlspecialchars($row['storyteller']) ?></span>
          </div>
          ...
          <?php if (in_array($row['id'], $purchased_stories)): ?>
          <button onclick="window.location.href='myStories.php'"
            class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition w-full justify-center">
            <i class="fas fa-play"></i> Read/Listen/Watch
          </button>
          <?php else: ?>
          <button onclick="purchaseStory(<?= $row['id'] ?>)"
            class="bg-gradient-to-r from-yellow-400 to-red-500 hover:from-yellow-500 hover:to-red-600 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition w-full justify-center">
            <i class="fas fa-shopping-cart"></i> Purchase
          </button>
          <?php endif; ?>
          <button
            class="block mt-4 w-full bg-blue-100 hover:bg-blue-200 text-blue-900 font-semibold py-1 rounded open-modal"
            data-story='<?= htmlspecialchars(json_encode($row), ENT_QUOTES, "UTF-8") ?>'>
            View Details
          </button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col">
        ...
        <div class="p-6 border-t border-gray-200 flex-shrink-0">
          <div class="flex items-center justify-between">
            <div id="modalFavoriteBtn" class="text-gray-400 hover:text-red-500 cursor-pointer transition text-2xl"
              title="Favorite">
              <i class="fas fa-heart"></i>
            </div>
            <div id="modalPurchaseBtn"
              class="bg-gradient-to-r from-yellow-400 to-red-500 hover:from-yellow-500 hover:to-red-600 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition">
          ...
          <div class="mt-4">
            ...
            <form id="commentForm" class="flex gap-2">
              <input type="text" id="commentInput" class="flex-1 border rounded px-2 py-1"
  ...
  function renderModal(story) {
    ...
    if (isPurchased) {
      ...
      if (story.media_type === 'audio') {
        mediaHtml =
          `<audio controls class="w-full mt-2"><source src="${story.media_url}" type="audio/wav"></audio>
        ...
        if (story.media_url.includes('youtube.com/embed/')) {
          mediaHtml =
            `<iframe class="w-full mt-2" style="height: 220px;" src="${story.media_url}" frameborder="0" allowfullscreen></iframe>`;
        } else {
          mediaHtml =
            `<video controls class="w-full mt-2" style="max-height:220px;"><source src="${story.media_url}" type="video/mp4"></video>
                        <a href="${story.media_url}" download class="inline-block mt-2 px-4 py-1 bg-green-600 text-white rounded hover:bg-green-700 transition text-sm"><i class="fas fa-download mr-1"></i>Download</a>`;
        }
      } else if (story.media_type === 'text') {
        mediaHtml =
          `<a href="${story.media_url}" target="_blank" class="text-blue-700 underline mt-2 inline-block">Read Full Story</a>`;
      }
    } else {
      ...
      if (story.media_type === 'audio') {
        ...
        const preview = story.description.length > 200 ? story.description.substring(0, 200) + '...' : story
          .description;
    ...
    if (isPurchased) {
      ...
      modalPurchaseBtn.className =
        'bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition';
      ...
      modalPurchaseBtn.className =
        'bg-gradient-to-r from-yellow-400 to-red-500 hover:from-yellow-500 hover:to-red-600 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition';
  ...
  function renderComments(comments) {
    ...
    list.innerHTML = comments.length ? comments.map(function(c) {
      return `<div class="bg-gray-100 rounded px-2 py-1 mb-1">${c}</div>`;
  ...
  document.getElementById('commentForm').onsubmit = function(e) {
    ...
    fetch('', {
        ...
        body: 'comment_story_id=' + encodeURIComponent(currentStory.id) + '&comment_text=' + encodeURIComponent(
          text)
      })
      .then(res => res.json())
      .then(data => {
