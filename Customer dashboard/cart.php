...
if ($story_cart) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM stories WHERE id IN ($story_ids)");
    while ($row = mysqli_fetch_assoc($result)) {
    ...
    <div class="container mx-auto px-4 py-10">
        ...
        <form method="post">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b">
                                    <td class="py-3 px-4 flex items-center gap-4">
                                        <img src="<?= htmlspecialchars($item['image'] ?: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=400&q=80') ?>" alt="Product" class="w-16 h-16 object-cover rounded shadow">
                                        <div>
                                            <div class="font-bold text-blue-900"><?= htmlspecialchars($item['name']) ?></div>
                                            <div class="text-xs text-gray-500"><?= htmlspecialchars($item['category']) ?> | <?= htmlspecialchars($item['location']) ?></div>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 font-semibold text-green-700">ETB <?= $item['price'] ?></td>
                                    <td class="py-3 px-4">
                                        <input type="number" name="qty[<?= $item['id'] ?>]" value="<?= $item['qty'] ?>" min="1" class="w-16 border rounded px-2 py-1 text-center">
                                    </td>
                                    <td class="py-3 px-4 font-semibold">ETB <?= number_format($item['subtotal'], 2) ?></td>
                                    <td class="py-3 px-4">
                                        <a href="cart.php?remove=<?= $item['id'] ?>" class="text-red-600 hover:text-red-800" title="Remove"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white rounded-xl shadow-lg">
                            ...
                            <tbody>
                                ...
                                <tr class="border-b">
                                    <td class="py-3 px-4 flex items-center gap-4">
                                        <img src="<?= htmlspecialchars($item['thumbnail'] ?: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=400&q=80') ?>" alt="Story" class="w-16 h-16 object-cover rounded shadow">
                                        <div>
                                            <div class="font-bold text-blue-900"><?= htmlspecialchars($item['title']) ?></div>
                                            <div class="text-xs text-gray-500">By <?= htmlspecialchars($item['storyteller']) ?> | <?= htmlspecialchars($item['category']) ?></div>
                                        </div>
                                    </td>
                                    <td class="py-3 px-4 font-semibold text-green-700">ETB <?= number_format($item['price'] ?? 0, 2) ?></td>
                                    <td class="py-3 px-4">
                                        <input type="number" name="story_qty[<?= $item['id'] ?>]" value="<?= $item['qty'] ?>" min="1" class="w-16 border rounded px-2 py-1 text-center">
                                    </td>
                                    <td class="py-3 px-4 font-semibold">ETB <?= number_format($item['subtotal'], 2) ?></td>
                                    <td class="py-3 px-4">
                                        <a href="cart.php?remove_story=<?= $item['id'] ?>" class="text-red-600 hover:text-red-800" title="Remove"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mt-8 gap-4">
                <div>
                    <button type="submit" name="update" class="bg-blue-700 hover:bg-blue-900 text-white px-6 py-2 rounded-lg font-semibold shadow transition">Update Cart</button>
                </div>
                <div class="text-xl font-bold text-blue-900">
                    Total: <span class="text-green-700">ETB <?= number_format($grand_total, 2) ?></span>
                </div>
                <div>
                    <a href="checkout.php" class="bg-gradient-to-r from-yellow-400 to-red-500 hover:from-yellow-500 hover:to-red-600 text-white px-8 py-3 rounded-lg font-bold shadow transition flex items-center gap-2">
