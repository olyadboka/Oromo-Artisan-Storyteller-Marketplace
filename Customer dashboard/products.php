...
$categories = $con->query("SELECT DISTINCT category FROM products");
$locations = $con->query("SELECT DISTINCT location FROM products");
$artisans = $con->query("SELECT DISTINCT artisan FROM products");
...
if ($search) $query .= " AND name LIKE '%" . mysqli_real_escape_string($con, $search) . "%'";
if ($category) $query .= " AND category='" . mysqli_real_escape_string($con, $category) . "'";
if ($location) $query .= " AND location='" . mysqli_real_escape_string($con, $location) . "'";
if ($artisan) $query .= " AND artisan='" . mysqli_real_escape_string($con, $artisan) . "'";
      ...
      <div
        ...
        <div class="p-5">
          ...
          <div class="flex items-center justify-between mb-2">
            ...
            <span
              class="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded"><?= htmlspecialchars($row['location']) ?></span>
          </div>
        ...
        <div
          class="absolute top-3 right-3 bg-white bg-opacity-80 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 shadow">
