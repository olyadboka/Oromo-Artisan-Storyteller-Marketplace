    ...
    <div class="container mx-auto px-4 py-10">
        ...
        <?php if (empty($purchased_stories)): ?>
            <div class="text-center py-12">
                ...
                <a href="storyLibrary.php" class="bg-gradient-to-r from-yellow-400 to-red-500 hover:from-yellow-500 hover:to-red-600 text-white px-6 py-3 rounded-lg font-semibold transition">
        ...
        <?php else: ?>
            ...
            <form method="get" class="flex flex-wrap gap-4 justify-center mb-8">
                ...
                <a href="myStories.php" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg font-semibold shadow transition">Clear</a>
            </form>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                ...
                <?php foreach($filtered_stories as $story): ?>
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden transform hover:-translate-y-2 hover:shadow-2xl transition relative group">
                        ...
                        <img src="<?= htmlspecialchars($story['thumbnail']) ?>" alt="Story" class="w-full h-40 object-cover group-hover:scale-105 transition">
                        <div class="p-5">
                            ...
                            <div class="flex items-center justify-between mb-2">
                                <span class="bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded"><?= htmlspecialchars($story['category']) ?></span>
                                <span class="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded"><?= htmlspecialchars($story['storyteller']) ?></span>
                            </div>
                            <div class="flex items-center justify-between mb-3">
                                <span class="text-sm text-gray-500"><?= ucfirst($story['media_type']) ?></span>
                                <span class="text-xs text-gray-400">Purchased: <?= date('M j, Y', strtotime($story['purchase_date'])) ?></span>
                            </div>
                            <button onclick="openStory(<?= htmlspecialchars(json_encode($story), ENT_QUOTES, "UTF-8") ?>)" class="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-4 py-2 rounded-lg font-semibold shadow flex items-center gap-2 transition justify-center">
