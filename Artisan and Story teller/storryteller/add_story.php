...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $language = $_POST['language'] ?? 'Afaan Oromo';
  ...
  $media_url = '';
  if (empty($errors) && isset($_FILES['media_file']) && $_FILES['media_file']['error'] === UPLOAD_ERR_OK) {
  ...
  if (empty($errors)) {
    try {
      ...
      $stmt = $conn->prepare("
                INSERT INTO stories (
                    storyteller_id, title, description, media_type, media_url, 
                    duration, word_count, language, age_group, is_featured
                ) VALUES (
                    :storyteller_id, :title, :description, :media_type, :media_url,
                    :duration, :word_count, :language, :age_group, :is_featured
                )
...
<head>
  ...
  <title>Add New Story - Oromo Storyteller Network</title>
